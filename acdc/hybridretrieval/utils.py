...
def get_all_hybrid_retrieval_things(num_examples, device, metric_name, kl_return_one_element=True):
    ...
    dataset = KBICRDataset(N=num_examples, seed=42)

    seq_len = dataset.toks.shape[1]

    clean_prompts = dataset.toks.long()[:num_examples, :seq_len - 1].to(device)
    corrupted_prompts = dataset.toks.long()[num_examples:, :seq_len - 1].to(device) 
    clean_prompts_correct_answers = dataset.toks.long()[:num_examples, seq_len - 1]
    clean_prompts_wrong_answers = torch.as_tensor(dataset.non_s_tokenIDs[:num_examples], dtype=torch.long)

    assert torch.equal(clean_prompts_correct_answers, torch.as_tensor(dataset.s_tokenIDs, dtype=torch.long))
    assert torch.equal(clean_prompts_wrong_answers, torch.as_tensor(dataset.non_s_tokenIDs, dtype=torch.long))
    ...
    # Split the data
    validation_data = clean_prompts[:validation_slice, :]
    validation_labels = clean_prompts_correct_answers[:validation_slice]
    validation_patch_data = corrupted_prompts[:validation_slice, :]
    validation_wrong_labels = clean_prompts_wrong_answers[:validation_slice]
    test_data = clean_prompts[validation_slice:, :]
    test_labels = clean_prompts_correct_answers[validation_slice:]
    test_patch_data = corrupted_prompts[validation_slice:, :]
    test_wrong_labels = clean_prompts_wrong_answers[validation_slice:]
